...
# Deployment script for tech-tip to a remote server
...
DEPLOY_DIR="/tmp/tech-tip-deploy"

echo -e "${BLUE}=== Tech Tip Generator - Deployment Script ===${NC}"
echo ""
...
read -p "Data directory [/var/lib/tech-tip]: " DATA_DIR
DATA_DIR="${DATA_DIR:-/var/lib/tech-tip}"
...
echo "  Binary path:   $INSTALL_DIR/tech-tip"
...
if [ ! -f "tech-tip" ]; then
    echo -e "${RED}Error: Binary 'tech-tip' not found${NC}"
    echo "Please run 'go build -o tech-tip' first"
    exit 1
fi
...
echo ""
echo "The tech-tip generator has been deployed to $REMOTE_HOST"
echo ""
echo "Installation details:"
echo "  Binary:   $INSTALL_DIR/tech-tip"
echo "  Database: $DATA_DIR/tech_tips.db"
echo ""
echo "To manage the service on the remote server:"
echo "  ssh $REMOTE_HOST 'systemctl --user status tech-tip.timer'"
echo "  ssh $REMOTE_HOST 'systemctl --user list-timers'"
echo "  ssh $REMOTE_HOST 'journalctl --user -u tech-tip.service'"
echo ""
echo "To run manually:"
echo "  ssh $REMOTE_HOST '$INSTALL_DIR/tech-tip'"
echo ""
