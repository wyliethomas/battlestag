...
# Remote installation script for stoic-thought
...
DATA_DIR="${2:-/var/lib/stoic-thought}"
OLLAMA_HOST="${3:-http://localhost:11434}"
OLLAMA_MODEL="${4:-dolphin3}"

BINARY_NAME="stoic-thought"
...
echo -e "${GREEN}=== Stoic Thought Generator - Remote Installation ===${NC}"
echo ""
...
if [ -f "stoic-thought.service" ] && [ -f "stoic-thought.timer" ]; then
    ...
    sed -e "s|ExecStart=.*|ExecStart=$INSTALL_DIR/$BINARY_NAME|g" \
        ...
        -e "s|Environment=\"DB_PATH=.*\"|Environment=\"DB_PATH=$DATA_DIR/thoughts.db\"|g" \
        stoic-thought.service > "$SYSTEMD_USER_DIR/stoic-thought.service"

    cp stoic-thought.timer "$SYSTEMD_USER_DIR/"
...
echo "   systemctl --user list-timers stoic-thought.timer"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo ""
    systemctl --user list-timers stoic-thought.timer
fi
